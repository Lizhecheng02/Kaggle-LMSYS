...
def use_tf_idf_vectorizer(train, column_name):
    ...
    with open(f"./vectorizers/tf_idf_vectorizer_{column_name}.pkl", "wb") as f:
        pickle.dump(vectorizer, f)

    return vectorizer, df


def use_count_vectorizer(train, column_name):
    ...
    with open(f"./vectorizers/count_vectorizer_{column_name}.pkl", "wb") as f:
        pickle.dump(vectorizer, f)

    return vectorizer, df


def use_tf_idf_vectorizer_for_test(vectorizer, test, column_name):
...
def use_count_vectorizer_for_test(vectorizer, test, column_name):
    X_test = vectorizer.transform([i for i in test[column_name]])
    dense_matrix = X_test.toarray()
    df = pd.DataFrame(dense_matrix)
    count_columns = [f"{column_name}_count_{i}" for i in range(len(df.columns))]
